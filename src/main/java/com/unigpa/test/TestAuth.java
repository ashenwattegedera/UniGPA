...
public class TestAuth {
    public static void main(String[] args) {
        ...
        if (userModules.size() == 1 && userModules.get(0).getModuleCode().equals("CS101")) {
            System.out.println("   [SUCCESS] Modules fetched correctly.");
        } else {
            System.err.println("   [FAILED] Module fetch incorrect. Found: " + userModules.size());
        }

        System.out.println("5. Verifying Data Isolation (fetching for random ID)...");
        List<Module> otherModules = moduleDAO.getModulesByUserId(user.getId() + 1); // Assuming random ID doesn't clash
                                                                                    // immediately
